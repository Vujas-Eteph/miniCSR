...
def thread_server_moni(START_TIME, HOURS_TO_RUN, cmd_manager, GPU_server_alias,
                       server_watcher, SLEEP, lock, barrier, shared_server_stats,
                       shared_server_stats_detailed, shared_server_disk_detailed):
    ...
    while time.time() - START_TIME < HOURS_TO_RUN:
        ...
        host_output, _ = \
            cmd_manager.execute_cmd_on_host(GPU_server_alias, 'nvidia-smi')
        cpu_stats = \
            [cmd_manager.execute_cmd_on_host(GPU_server_alias, cmd)[0] for cmd in ["free",
                                                                                   "uptime",
                                                                                   "ps -eo user,pcpu",
        ...
        disk_space_stats = \
            [cmd_manager.execute_cmd_on_host(GPU_server_alias, cmd)[0] for cmd in ["df | grep -E '^/dev/' | grep -vE 'boot|tmpfs'"]
             ]

        # Thread-safe write to results
        gpac_msg = server_watcher.get_server_statistics(GPU_server_alias,
...
def main():
    ...
    def update_latest_results():
        ...
        with results_lock:
            ...
            df_stats = pd.DataFrame.from_dict(shared_server_stats, orient='index')
            df_stats_detailed = pd.DataFrame.from_dict(shared_server_stats_detailed, orient='index')
            df_disk_space_detailed = pd.DataFrame.from_dict(shared_server_disk_detailed, orient='index')
            ...
            latest_result += tabulate(df_stats_sorted, headers='keys', tablefmt='grid')

            latest_result_detailed = f"Time Stamp: {current_time}\n"  # Add timestamp
            latest_result_detailed += tabulate(df_stats_detailed_sorted, headers='keys', tablefmt='grid')

            latest_result_detailed_space = f"Time Stamp: {current_time}\n"  # Add timestamp
            latest_result_detailed_space += tabulate(df_disk_space_detailed_sorted, headers='keys', tablefmt='grid')
        ...
        for GPU_server_alias in GPU_SERVER_ALIASES:
            thread = threading.Thread(
                ...
                args=(START_TIME, HOURS_TO_RUN, cmd_manager, GPU_server_alias,
                      server_watcher, SLEEP, results_lock, barrier, shared_server_stats,
                      shared_server_stats_detailed, shared_server_disk_detailed)
            )
