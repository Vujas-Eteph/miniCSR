...
class NvidiaSmiPatternExtractor():
    ...
    def _init_regex_pattern(self):
        ...
        self.gpu_name_pattern = \
            re.compile(r"\|\s+(\d+)\s+([A-Za-z0-9\s\-]+?)(?:\s+Off)?\s+\|")
        self.fan_temp_perf_pattern = \
            re.compile(r"\|\s+(N/A|\d+)%?\s+(\d+)C\s+([A-Za-z0-9]+)\s+(\d+)W\s+/\s+(\d+)W\s+\|")
        self.memory_pattern = \
            re.compile(r"\|\s+(\d+)MiB\s+/\s+(\d+)MiB\s+\|")
        self.gpu_util_pattern = \
            re.compile(r"\|\s+(\d+)%\s+Default\s+\|")
