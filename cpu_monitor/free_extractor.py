...
class FreePatternExtractor:
    ...
    def _init_regex_pattern(self):
        ...
        self.swap_pattern = re.compile(r"Swap:\s+(\d+)\s+(\d+)\s+(\d+)")
    ...
    def _get_mem_info(self, output):
        ...
        if memory_match:
            ...
            buffer_cache = int(
                memory_match.group(4)
            )  # Buffer/cache memory in MiB
        else:
            raise ValueError(
                "Could not find memory information in free output."
            )
    ...
    def get_free_info(self, free_output):
        ...
        total_swap, used_swap, free_swap = self._get_swap_info(free_output)
